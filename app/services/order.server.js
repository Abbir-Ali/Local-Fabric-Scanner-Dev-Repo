  ...
  try {
    ...
    if (isBinSearch && query) {
      ...
      edges = edges.filter((edge) => {
        ...
        const binValue = (binMetafield?.node.value || '').toLowerCase().trim();

        // Debug: Log all BIN values we encounter
        if (binValue) {
          console.log(`[BIN VALUE FOUND] Product "${edge.node.title}" has BIN: "${binValue}" (original: "${binMetafield?.node.value}")`);
        }
